...
export VLLM_API_KEY=...            # vLLM (optional, for secured local servers)
export LITELLM_API_KEY=...         # LiteLLM proxy (optional, if proxy requires auth)
...
We built TradingAgents with LangGraph to ensure flexibility and modularity. The framework supports multiple LLM providers: OpenAI, Google, Anthropic, xAI, DeepSeek, Qwen (Alibaba DashScope), GLM (Zhipu), OpenRouter, Ollama and vLLM for local models, LiteLLM proxy routing, and Azure OpenAI for enterprise.
...
config["llm_provider"] = "openai"        # openai, google, anthropic, xai, deepseek, qwen, glm, openrouter, ollama, vllm, litellm, azure
